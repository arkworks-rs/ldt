    ...
    S::Config: Clone,
{
    ...
    fn fold(&mut self) {
        ...
        if !self.is_final_round() {
            ...
            let evals = folded_coeff
                ...
                .evals;
            let folded_committed_values = stack_evaluations(evals, self.config.folding_factor);
    ...
    fn update_challenges(&mut self) {
        ...
        self.challenges =
            dedup((0..self.config.num_repetitions[self.round_num]).map(|_| {
                squeeze_integer(&mut self.sponge, domain_size / self.config.folding_factor)
            }));
    ...
    fn update_out_of_domain_samples(&mut self) {
        self.out_of_domain_samples =
            self.sponge_squeeze_multiple(self.config.num_out_of_domain_samples);
    ...
    fn update_proof_of_work(&mut self) {
        self.proof_of_work_nonce = proof_of_work(
            &mut self.sponge,
            self.config.num_proof_of_work_bits[self.round_num],
        );
    }
